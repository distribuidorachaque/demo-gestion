...
// Todo lo "propio" del negocio (nombre, logo, alias de pago, contacto) vive acá
// y se edita desde la pantalla Configuración, SIN tocar el código. En esta
// versión DEMO arranca en blanco para que la cargue el negocio nuevo.
...
function getLogoTicket() {
  return (configNegocio.logo && configNegocio.logo.length > 20) ? configNegocio.logo : ("data:image/png;base64," + LOGO_TICKET_BASE64);
}
  ...
  try {
    ...
    try {
      const [rC0, rCat0, rP0, rI0] = await Promise.all([
        backendFetch(API_URL + "?tipo=clientes"),
        backendFetch(API_URL + "?tipo=catalogo"),
        backendFetch(API_URL + "?tipo=pedidos"),
        backendFetch(API_URL + "?tipo=inversiones")
      ]);
    ...
    for (let intento = 1; intento <= maxIntentos; intento++) {
      ...
      try {
        await Promise.all([
          backendFetch(API_URL, { method: "POST", mode: "no-cors", body: JSON.stringify({ tipo: "clientes", payload: clients }) }),
          backendFetch(API_URL, { method: "POST", mode: "no-cors", body: JSON.stringify({ tipo: "catalogo", payload: catalog }) }),
          backendFetch(API_URL, { method: "POST", mode: "no-cors", body: JSON.stringify({ tipo: "pedidos",  payload: orders  }) }),
          backendFetch(API_URL, { method: "POST", mode: "no-cors", body: JSON.stringify({ tipo: "inversiones", payload: inversiones }) })
        ]);
...
function guardarCliente() {
  ...
  const telefono2  = document.getElementById("formClienteTelefono2")?.value.trim() || "";
...
function abrirWhatsAppCliente2(clienteId) {
  const c = clients.find(c => c.id === clienteId);
  if (!c || !c.telefono2) return;
  const tel = c.telefono2.replace(/\D/g, "");
  window.open(`https://wa.me/54${tel}`, "_blank");
}
...
async function sincronizarConfigVehiculo() {
  try {
    const r = await backendFetch(API_URL + "?tipo=vehiculo");
...
// El alias de pago ahora vive en configNegocio.aliasPago (pantalla Configuración).
...
function enviarRecordatorioPago(clienteId) {
  ...
  const msg = `Hola ${client.nombre} 👋\n\nTe escribo para recordarte que tenés un saldo pendiente de *${formatCurrency(deuda)}* con ${configNegocio.nombre}.\n\nPodés transferir al alias: *${configNegocio.aliasPago}*\n\nCuando puedas, coordinamos el pago. ¡Gracias! 🙌`;
...
function copiarAlias() {
  navigator.clipboard.writeText(configNegocio.aliasPago).then(() => {
    alert("✅ Alias copiado: " + configNegocio.aliasPago);
  }).catch(() => {
    alert("No se pudo copiar automáticamente. Tu alias es: " + configNegocio.aliasPago);
  });
}
...
function renderApp() {
    ...
    <div class="modal-overlay" id="modalCliente" style="display:none;" onclick="cerrarModal()">
      <div class="modal-card" onclick="event.stopPropagation()">
        ...
        <div class="modal-grid">
          ...
          <button class="modal-btn" id="btnModalTelefono2" style="display:none;" onclick="abrirWhatsAppCliente2(clienteActivoId)">
            <span class="modal-icon">💬</span>
            <span>WhatsApp Tel. 2</span>
          </button>
  ...
  ${contarStockBajo() > 0 ? `<span class="nav-badge">${contarStockBajo()}</span>` : ''}
      ...
      <button class="nav-btn ${vistaActual === 'config' ? 'active' : ''}" onclick="setVista('config')">
  ...
  switch (vistaActual) {
    ...
    case "config":     renderVistaConfig();     break;
  }
}

// ── Vista: CONFIGURACIÓN DEL NEGOCIO ─────────────────────────────────────────
function renderVistaConfig() {
  const c = configNegocio;
  const esc = (x) => (x || "").replace(/"/g, "&quot;");
  document.getElementById("vista-contenido").innerHTML = `
    <div class="page-header">
      <h1 class="page-title">⚙️ Configuración</h1>
      <p class="muted">Estos son los datos de tu negocio. Se usan en el título de la app, en los remitos (PDF), en los tickets y en los mensajes de WhatsApp.</p>
    </div>

    <div class="form-card">
      <label>Nombre del negocio</label>
      <input id="cfgNegNombre" type="text" value="${esc(c.nombre)}" placeholder="Ej: Mi Distribuidora" />

      <label>Ciudad</label>
      <input id="cfgNegCiudad" type="text" value="${esc(c.ciudad)}" placeholder="Ej: Santa Fe, Argentina" />

      <label>Teléfono de contacto</label>
      <input id="cfgNegTelefono" type="text" value="${esc(c.telefono)}" placeholder="Ej: 342-5040728" />

      <label>Email de contacto</label>
      <input id="cfgNegEmail" type="text" value="${esc(c.email)}" placeholder="Ej: minegocio@gmail.com" />

      <label>Alias para cobrar (transferencias)</label>
      <input id="cfgNegAlias" type="text" value="${esc(c.aliasPago)}" placeholder="Ej: mi.alias.mp" />

      <label>Logo del negocio</label>
      <div style="display:flex; align-items:center; gap:12px; margin:6px 0 12px;">
        <img id="cfgNegLogoPreview" src="${getLogoFactura()}" style="width:64px; height:64px; object-fit:contain; border:1px solid #ddd; border-radius:8px; background:#fff;" />
        <div>
          <input id="cfgNegLogoInput" type="file" accept="image/*" onchange="subirLogoNegocio(event)" />
          <div class="muted" style="font-size:12px;">Se usa en el remito PDF y en el ticket. Cuadrado se ve mejor.</div>
        </div>
      </div>
...
function subirLogoNegocio(event) {
  ...
  const reader = new FileReader();
  reader.onload = () => {
    const preview = document.getElementById("cfgNegLogoPreview");
    if (preview) { preview.src = reader.result; preview.dataset.nuevoLogo = reader.result; }
  };
...
function guardarConfigNegocio() {
  const preview = document.getElementById("cfgNegLogoPreview");
  const nuevoLogo = (preview && preview.dataset.nuevoLogo) ? preview.dataset.nuevoLogo : configNegocio.logo;
  configNegocio = {
    ...configNegocio,
    nombre:    document.getElementById("cfgNegNombre").value.trim() || configNegocio.nombre,
    ciudad:    document.getElementById("cfgNegCiudad").value.trim(),
...
function renderVistaClientes() {
  ...
  cont.innerHTML = `
    <div class="page-header" style="flex-direction:column; align-items:stretch; gap:10px;">
      <div>
        <h1 class="page-title">🪵 ${configNegocio.nombre}</h1>
  ...
  cont.innerHTML = `
    ...
    <div class="form-card">
      ...
      <div class="form-group">
        ...
        <input id="formClienteTelefono2" placeholder="Ej: 3415005678" value="${c ? c.telefono2 || "" : ""}" />
      </div>
      <div class="form-group">
        <label>Email</label>
...
let mostrarClientesHoy = false;
function toggleClientesHoy() {
  mostrarClientesHoy = !mostrarClientesHoy;
  renderVistaResumen();
}
  ...
  cont.innerHTML = `
    ...
    <div class="form-card">
      ...
      <div class="stats-grid">
        <div class="stat-box" onclick="toggleClientesHoy()" style="cursor:pointer;">
          <div class="muted">Pedidos hoy ${mostrarClientesHoy ? "▲" : "▼"}</div>
      ...
      ${mostrarClientesHoy ? (
        pedidosHoy.length === 0
          ? `<p class="muted" style="margin:10px 0 0;">Todavía no cargaste pedidos hoy.</p>`
          : `<div style="margin-top:10px; border-top:1px solid #e5e7eb; padding-top:8px;">
              ${pedidosHoy.map(o => `
                <div class="client-item" onclick="abrirModalCliente('${o.client.id}')">
                  <div class="client-info">
                    <div class="client-name">${o.client.nombre}</div>
                    <div class="client-sub">${formatCurrency(o.modoPrecio === "con" ? o.totals.totalConIVA : o.totals.totalSinIVA)}</div>
                  </div>
                  <span class="client-arrow">›</span>
                </div>
              `).join("")}
...
async function generarPDFListaPrecios() {
  ...
  doc.text(configNegocio.nombre, margin, y - 2);
...
// Logo de Distribuidora Chaque en color, para el encabezado del PDF de venta.
const LOGO_FACTURA_BASE64 = "data:image/png;base64,";
...
async function generarFacturaPDF(order, modo) {
  ...
  // Logo de Distribuidora Chaque, arriba a la izquierda
  doc.addImage(getLogoFactura(), "PNG", margin, y - 6, 20, 20);
  doc.setFont("helvetica", "bold"); doc.setFontSize(16);
  doc.text(configNegocio.nombre, margin + 24, y + 4);
  ...
  y += 20;
  doc.setFont("helvetica", "normal"); doc.setFontSize(9);
  doc.text(configNegocio.ciudad, margin, y); y += 4;
  doc.text("Tel: " + configNegocio.telefono, margin, y); y += 4;
  doc.text(configNegocio.email, margin, y); y += 6;
  ...
  doc.text(`${configNegocio.nombre} · ${configNegocio.ciudad}`, pageW / 2, pageH - 9, { align: "center" });
...
function _crearExcelModelo() {
  ...
  marcas.forEach(marca => {
    ...
    rows.push([configNegocio.nombre,"","","","","","","","","Costo total",{f:`SUM(F${dataRow}:F${lastRow})`}]);
